...
class DanmakuClient:
    ...
    def __init__(
        ...
        *,
        ...
        """消息处理器"""
        self._get_reconnect_interval: Callable[[int, int], float] = (
    ...
    async def _on_before_ws_connect(self, retry_count: int):
        ...
        if retry_count > 0 and retry_count % reinit_period == 0:
            logger.debug(
                "room=%d reinitializing room after %d retries",
                self.room_id,
                retry_count,
            )
    ...
    async def _on_ws_message(self, message: aiohttp.WSMessage):
        ...
        if message.type != aiohttp.WSMsgType.BINARY:
            logger.warning(
                "room=%d unknown websocket message type=%s", self.room_id, message.type
            )
            return
    ...
    async def _parse_ws_message(self, data: bytes):
        ...
        if header.operation in (Operation.SEND_MSG_REPLY, Operation.AUTH_REPLY):
            ...
            # 转换为业务消息处理
            command = {"cmd": "_HEARTBEAT", "data": {"popularity": popularity}}
            self._handle_command(command)

        else:
            # 未知消息
            logger.warning(
                "room=%d unknown message operation=%d", self.room_id, header.operation
            )
    ...
    async def _parse_business_message(self, header: HeaderTuple, body: bytes):
        ...
        if header.operation == Operation.SEND_MSG_REPLY:
            ...
            if header.ver == ProtoVer.BROTLI:
                ...
                logger.warning(
                    "room=%d unknown protocol version=%d", self.room_id, header.ver
                )
            ...
            if command["code"] != AuthReplyCode.OK:
                raise AuthError(
                    f"auth reply error, code={command['code']}, body={command}"
                )
            # 认证成功后立即发送心跳
            if self._websocket is None:
                raise RuntimeError("room=%d websocket is None", self.room_id)
            await self._websocket.send_bytes(make_packet({}, Operation.HEARTBEAT))
    ...
    async def _init_uid(self) -> bool:
        ...
        try:
            async with self._session.get(
                ...
                if res.status != 200:
                    logger.warning(
                        "room=%d _init_uid() failed, status=%d",
                        self._tmp_room_id,
                        res.status,
                    )
                    return False
    ...
    async def _init_buvid(self) -> bool:
        ...
        try:
            async with self._session.get(
                ...
                if res.status != 200:
                    logger.warning(
                        "room=%d _init_buvid() status error, status=%d",
                        self._tmp_room_id,
                        res.status,
                    )
        except (aiohttp.ClientConnectionError, asyncio.TimeoutError):
    ...
    async def _init_room_id_and_owner(self) -> bool:
        ...
        try:
            async with self._session.get(
                ...
                if res.status != 200:
                    logger.warning(
                        "room=%d _init_room_id_and_owner() failed, status=%d",
                        self._tmp_room_id,
                        res.status,
                    )
                    return False
    ...
    async def _init_host_server(self) -> bool:
        ...
        if self._wbi_signer.need_refresh_wbi_key:
            ...
            if self._wbi_signer.wbi_key == "":
                logger.warning(
                    "room=%d _init_host_server() failed: no wbi key", self._room_id
                )
                return False
        ...
        try:
            async with self._session.get(
                ...
                if res.status != 200:
                    logger.warning(
                        "room=%d _init_host_server() failed, status=%d",
                        self._room_id,
                        res.status,
                    )
                    return False
    ...
    def _parse_danmaku_server_conf(self, data: dict) -> bool:
        ...
        if not self._host_server_list:
            logger.warning(
                "room=%d _parse_danmaku_server_conf() failed: host_list is empty",
                self._room_id,
            )
            return False
        logger.debug(
            "room=%d got %d host servers", self._room_id, len(self._host_server_list)
        )
        return True
    ...
    def _get_ws_url(self, retry_count: int) -> str:
        ...
        if self._host_server_list is None:
            raise RuntimeError("room=%d host_server_list is None", self._room_id)
